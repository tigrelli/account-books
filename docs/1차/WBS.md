# 지출 관리 가계부 — WBS (Work Breakdown Structure) — 1차 개발(완료)

> ⚠️ **1차 개발 완료 — 2026-07-10**
> 이 문서는 1차(메인앱 코어 기능 + 운영 배포) 설계 당시 작성된 원본 계획입니다. 실제 완료 이력은 같은 폴더의 `진행현황.md`(1차본)를 기준으로 합니다.
>
> 아래 계획 중 다음 항목은 **1차 범위에서 제외되어 2차 개발로 이관**되었습니다.
>
> - **Phase 2 — 서브앱(관리비) Webhook 연동** (전체)
> - **Phase 4 — 서브앱 확장 + 계정연결 정식화** (전체)
> - **S-5-2 모니터링/에러로깅 설정** (Phase 5 내)
> - **S-1-13 소셜 로그인(OAuth Provider) 설정** (Phase 1 내, 코드는 구현되어 있으나 실사용 검증 미완료 — 2차 마지막 TASK로 배치)
>
> 2차의 정식 작업계획서/WBS는 **관리비 명세서 설계(데이터 구조·화면설계·기능명세) 완료 후** `docs/2차/`에 새로 작성됩니다. 아래 Phase 2/4 섹션은 그 설계의 출발점이 되는 **초기 스케치**로만 참고하십시오. 이관된 항목의 상세 이력·원본 TASK 목록은 `docs/2차/인수인계.md`에 보존되어 있습니다.
...
| Phase 2 | 2단계                | 서브앱 1개(관리비) Webhook 연동 — **2차로 이관**         |
| Phase 3 | 3단계                | 통계 분리/캐시/대시보드 고도화                           |
| Phase 4 | 4단계                | 서브앱 확장(구독/카드), 계정연결 정식화 — **2차로 이관** |
...
| S-1-13  | 시스템 | 소셜 로그인(OAuth Provider) 설정 (선택) — **2차로 이관** | 3    | S-1-12        | 0.4  |
...
## 4. Phase 2 — 서브앱(관리비) Webhook 연동 (2단계) — **⚠️ 전체 2차로 이관, 초기 스케치 참고용**

> 아래 표는 1차 설계 당시 작성된 초안입니다. 관리비 명세서 데이터/화면 설계가 끝난 뒤 이 내용을 출발점으로 재배치·재분할하여 `docs/2차/WBS.md`를 새로 작성합니다. 원문은 그대로 보존만 합니다.
...
### 4-4. 관리비 서브앱 (별도 작업물, 최소 구현 — 관리비 명세서 설계로 대체될 예정)
...
## 6. Phase 4 — 서브앱 확장 + 계정연결 정식화 (4단계) — **⚠️ 전체 2차로 이관, 초기 스케치 참고용**
...
| S-5-2   | 시스템 | 모니터링/에러로깅 설정(선택) — **2차로 이관**       | 8-1            | S-5-1          | 0.4  |
...
| Phase 2 서브앱 Webhook 연동 | 9.8             | (2차로 이관)                                   |
| Phase 3 통계/캐시 고도화    | 7.7             | 6일                                            |
| Phase 4 서브앱 확장         | 3.3             | (2차로 이관)                                   |
| Phase 5 통합테스트/QA/출시  | 3.4             | 3일                                            |
| **총합**                    | **약 58.0세션** | **약 48일(영업일 기준, 주말 제외 시 약 10주)** |

> 이 추정은 "TASK를 세션 단위로 끝낼 수 있게 잘게 쪼갠다"는 전제 위에서 나온 숫자이며, 실제로는 ① Claude Pro 세션당 처리량이 작업 복잡도/모델 선택에 따라 더 적거나 많을 수 있고 ② 1인 PM이 직접 검수/디버깅하는 시간(Claude 세션 외 시간)은 포함되지 않았습니다.

---

## 9. 작업 순서 검토 — 권장 진행 순서 핵심 원칙

1. **DB(S) 먼저, 화면(F) 나중**: 모든 Phase에서 테이블/RLS 마이그레이션을 먼저 끝내고 화면을 붙입니다.
2. **마스터 데이터(F-1-1-1~3) → 지출 입력(F-1-1-4~10) → 대시보드(F-5-x) 순서 고정**.
3. **Phase 1의 "3-5. 지출 입력" 블록(8세션)이 최대 리스크 구간**입니다.
...
5. **서브앱(Phase 2, 4)은 메인앱 Webhook 수신부(S-2-2~5)가 먼저 완성된 후** 착수합니다.
6. **테스트(T-코드)는 각 기능 블록이 끝날 때마다 바로 진행**.
