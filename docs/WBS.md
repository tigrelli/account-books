...
| F-1-4-2 | 앱기능 | 항목분류 관리 화면 (CRUD, 부모-자식 계층 UI)         | F-1-1-2   | S-1-1, S-1-9 | 0.7  |
...
| TASK ID  | 구분   | 작업명                                                         | F-코드     | 선행작업          | 세션 |
| -------- | ------ | -------------------------------------------------------------- | ---------- | ----------------- | ---- |
| F-1-5-1  | 앱기능 | 지출 입력 폼 기본 골격 (지출분류/항목분류/지출처 선택 UI)      | F-1-1-4    | F-1-4-1~3         | 0.6  |
...
| TASK ID | 구분   | 작업명                             | F-코드 | 선행작업       | 세션 |
| ------- | ------ | ---------------------------------- | ------ | -------------- | ---- |
| F-1-7-1 | 앱기능 | 예산 설정 화면(카테고리별 월 한도) | F-4-1  | S-1-8, F-1-4-2 | 0.5  |
...
**Phase 1 총합: 약 25.7세션 → 1일 1.2세션 가정 시 약 21일(영업일 기준, 주말 제외 시 약 4.5주)**
...
| F-3-1-1 | 앱기능 | 지출분류별(①) 통계 화면 (현금/카드, 카드사별)                          | F-5-4       | S-3-1        | 0.5  |
...
2. **마스터 데이터(F-1-1-1~3) → 지출 입력(F-1-1-4~10) → 대시보드(F-5-x) 순서 고정**: 지출 입력 화면은 지출분류/항목분류/지출처가 먼저 있어야 선택 UI를 만들 수 있고, 대시보드는 지출 데이터가 쌓여야 검증 가능합니다.
