...
이 7개 문서가 이 프로젝트의 단일 진실 공급원(Source of Truth)입니다. 단, **이미 핵심 결정사항은 이 CLAUDE.md 안에 요약되어 있으므로, 대부분의 TASK는 이 파일만 보고 진행 가능**합니다. 원본 문서는 "이 파일에 없는 세부사항"이 필요할 때만 찾아봅니다.
...
| `docs/백로그.md`                   | **당장 하지 않는 기능/개선 아이디어 기록**. WBS에 아직 없는 것들 — 형식은 문서 상단 참고. PM이 착수 결정하면 WBS.md로 정식 승격                                     | 짧음                         |
...
- **CSS 변수 — 반드시 하드코딩 HEX 대신 변수명 사용**:
  ...
  | `--color-data-soft` | `#EA580C` | **보조 데이터** — 중립 배지(▼), 잔여 예산 바 |
  | `--color-action` | `#0D9488` | **UI 액션 전용** — CTA 버튼, 링크, 사이드바 활성 탭 |
  | `--color-paylens-cash` | `#43A047` | **지출분류 태그 전용 — 현금**(2026-07-03 추가) — 다른 UI 요소와 색을 공유하지 않음 |
  | `--color-paylens-check` | `#1E88E5` | **지출분류 태그 전용 — 체크카드**(2026-07-03 추가) |
  | `--color-paylens-credit` | `#FB8C00` | **지출분류 태그 전용 — 신용카드**(2026-07-03 추가) |
...
8. **작업 중 "당장 하진 않지만 나중에 검토할" 기능/개선 아이디어가 나오면, WBS.md에 바로 추가하지 말고 `docs/백로그.md`에 문서 상단 형식대로 기록할 것.** PM이 착수를 결정한 시점에만 WBS.md로 정식 TASK ID를 부여해 옮기고, 백로그 항목은 상태를 `채택`으로 갱신한다.
...
**현재 Phase: Phase 5 — 서브앱 확장** (`docs/진행현황.md` 챕터 7 — 원 WBS의 Phase 2 "서브앱 Webhook 연동"과 Phase 4 "서브앱 확장"을 통합해 순서를 뒤로 미룬 것. Phase 0/1(메인앱 Core)/3(통계·캐시)/4(통합테스트·QA·프로덕션 배포, 서브앱 전)까지 완료, 2026-07-09 PM 확인) (PM이 다음 Phase로 넘어갈 때 이 줄만 직접 고쳐서 갱신함 — Claude Code가 스스로 바꾸지 않음)

다음에 진행할 TASK는 `docs/진행현황.md`에서 **체크 안 된(`[ ]`) 항목 중 가장 위에 있는 것**부터 순서대로. 선행 TASK가 완료되지 않은 상태로 후속 TASK를 시작하지 말 것 — 순서가 바뀌면 재작업 위험이 커짐(WBS 9장 작업순서 원칙 참고).
